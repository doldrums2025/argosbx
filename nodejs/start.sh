...
# [安全修复] 从 Xray 官方 GitHub releases 下载 + SHA256 校验
...
if [ -n "$expected_sha256" ]; then
  ...
  if [ "$expected_sha256" = "$actual_sha256" ]; then
    ...
    echo "SHA256 校验失败 ❌"; rm -f "$tmpxray"; exit 1
  fi
else
...
echo
echo "=========启用Sing-box内核========="
# [安全修复] 从 Sing-box 官方 GitHub releases 下载 + SHA256 校验
if [ ! -e "$HOME/agsbx/sing-box" ]; then
...
if [ -n "$expected_sha" ]; then
  ...
  if [ "$expected_sha" = "$actual_sha" ]; then
    ...
    echo "SHA256 校验失败 ❌"; rm -f "$tmpsb"; exit 1
  fi
else
  echo "警告：无法获取校验文件，跳过校验"
fi
